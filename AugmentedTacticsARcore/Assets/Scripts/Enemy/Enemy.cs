...
public class Enemy : Actor
{
    ...
    private bool moveEnemy(Actor target)
    {
        ...
        //Debug.Log("Dist = " + Vector3.Distance(getMapPosition(), target.getMapPosition()) + " " + getMapPosition() + target.getMapPosition());
        //if (Vector3.Distance(getMapPosition(), target.getMapPosition()) <= 1)
        //    Attack(target);
        NextTurn();
        return isFinshed;
    }
